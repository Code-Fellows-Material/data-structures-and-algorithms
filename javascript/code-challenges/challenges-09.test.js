...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((prev, current) => current > prev ? current : prev);
...
const checkValues = (obj, value) => {
  let check = false;
  Object.values(obj).forEach( val => val === value && (check = true)); 
  return check;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  let flag = false;
  arr.forEach( charObj =>{
    if(charObj.hasOwnProperty('children')) parrentArr.push(Object.values(charObj)[0]);
  });
  parrentArr.forEach( parent => parent === character && (flag = true))
  return flag;
};
