...
const toLastNames = people => {
  let newArray = [];
  newArray = people.map(person => `${person.firstName} ${person.lastName}`);
  return newArray;
};
...
const addPurchases = (arr) => {
  let total;
  total = arr.reduce((acc, val) => acc + val.purchasePrice, 0);
  return total;
};
...
const countNumberOfElements = (arr) => {
  let total;
  total = arr.reduce((countTotal) => ++countTotal);
  return total;
};
...
const returnNames = (arr) => {
  let newArray = [];
  newArray = arr.reduce((acc, person) => [...acc, person.name], []);
  return newArray;
};
...
const reversedString = (str) => {
  ...
  let newString = '';
  newString = strArray.reduce((accString, letter) => {
    return letter + accString 
  }, '');
  return newString;
};
...
const countNumberOfChildren = (arr) => {
  let total;
  total = arr.reduce((childrenTotal, character) => character.children ? childrenTotal += character.children.length : childrenTotal, 0);
  return total;
};
...
const countPrimeNumbers = (arr) => {
  let total;
  total = arr.reduce( (count, num) => isPrime(num) ? ++count : count, 0);
  return total;
};
...
const extractStat = (statName, arr) => {
  ...
  let newArray = [];
  newArray = arr.reduce((acc, statObj) => statObj.stat.name ? statName === statObj.stat.name ? statObj : acc : null, {});
