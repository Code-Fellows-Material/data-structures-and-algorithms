...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsPeople.sort((personA, personB) => personB.height - personA.height);
...
const listFoods = (recipe) => {
  ...
  for(let item of newArr){
    newerArr.push(item.slice(item.indexOf(' ') + 1));
  }
  for(let item of newerArr){
...
const splitFoods = (recipe) => {
  ...
  for(let item of newArr){
    ...
    if(tempArr[3] && tempArr[4]){
      result.push(`${tempArr[2]} ${tempArr[3]} ${tempArr[4]}`);
    } 
    else if(tempArr[3]){
      result.push(`${tempArr[2]} ${tempArr[3]}`);
    } 
    else {
      result.push(tempArr[2]);
    }
  }
  return result;
};
