...
const screenForNames = (arr) => {
  let newArr = [];
  let validator = /^(Mr.|Mrs.|Ms.|Dr.)(\s{1})[A-Za-z]*$/g;
  newArr = arr.filter(str => validator.test(str));
  return newArr
}
...
const toTitleCase = (arr) => arr.map(str => [ str[0].toUpperCase(), ...str.substring(1)].join(''));
...
let biggerThanLuke = (arr) => {
    ...
    return arr.filter(person => ~~luke.mass < ~~person.mass).map( pers => pers.name).join(' - ');
};
...
function helpCheck(row1, col1, row2, col2, row3, col3){
  if(row1[0] === row1[1] && row1[1] === row1[2] && row1[2] !== "") return true;
  if(row2[0] === row2[1] && row2[1] === row2[2] && row2[2] !== "") return true;
  if(row3[0] === row3[1] && row3[1] === row3[2] && row3[2] !== "") return true;

  if(col1[0] === col1[1] && col1[1] === col1[2] && col1[2] !== "") return true;
  if(col2[0] === col2[1] && col2[1] === col2[2] && col2[2] !== "") return true;
  if(col3[0] === col3[1] && col3[1] === col3[2] && col3[2] !== "") return true;


  if(row1[0] === row2[1] && row2[1] === row3[2] && row3[2] !== "") return true;
  if(row1[2] === row2[1] && row2[1] === row3[0] && row3[0] !== "") return true;
...
const detectTicTacToeWin = (board) => {
  console.log()
  return helpCheck(board[0], [board[0][0], board[1][0], board[2][0]], board[1], [board[0][1], board[1][1], board[2][1]], board[2], [board[0][2], board[1][2], board[2][2]])
};
