...
const count = (target, input) => {
  let countArr = [];
  input.forEach(arr => {
    let tempArr = arr.filter( val => val === target && val);
    countArr = [...tempArr, ...countArr];
  })
...
const totalSum = (input) => {
  let sum = 0;
  input.forEach(arr => {
    let tempSum = arr.reduce( (prev, current) => prev + current);
    sum+=tempSum;
  })
  return sum
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let expArr = [];
  input.forEach(arr => {
    let tempArr = arr.filter( val => typeof val === 'number' && val % 5 === 0);
    filteredArr.push(tempArr);
  })
  filteredArr.forEach(arr => {
    let tempArr = arr.map(val => 2**val);
    expArr.push(tempArr);
  })
  return expArr
};
...
let findMaleAndFemale = (data) => {
  let tempArr = [];
  data.forEach(obj => {
    if(obj.gender === 'male' || obj.gender === 'female')  tempArr.push(obj.name);
  })
...
let findShortest = (data) => data.reduce((prev, current) => current.height < prev.height ?  prev : current).name;
