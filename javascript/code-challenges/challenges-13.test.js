...
const longestString = (arr) => {
  ...
  let bigIdx = 0;
  arr.reduce((prev, current, currentIdx) => {
...
const standardizePhoneNumbers = (arr) => arr.map(str => [...str].filter(char => /^\d$/.test(char)).join(''));
...
const onlyOddChars = str => [...str].filter((char, idx) => idx % 2 !== 0).join('');
...
const allHappy = (arr) => {
  let testArr = arr.filter(str => str.includes(':)'));
