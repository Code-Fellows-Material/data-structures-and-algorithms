...
const typeNum = (arr) => {
  const newArray = arr.filter(val => typeof val === 'number');
  return newArray;
};
...
const containsAnd = (arr) => {
  const newArray = arr.filter(val => val.includes('and'));
  return newArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArray = arr.filter(val => {
    ...
    for(let forb of forbiddenValues){
      forb === val && (matchFound = true);
    }
    return !matchFound
  });
  return newArray;
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const newArray = arr.filter(stat => stat.baseStat > minBaseStat);
  return newArray;
};
...
const getCharactersWithoutChildren = (arr) => {
  const newArray = arr.filter(val => !val.hasOwnProperty('children'));
  return newArray;
};
...
const evenOddNumericValues = (arr) => {
  const newArray = arr.filter(val => typeof val === 'number');
  const mapArray = newArray.map(val => val % 2 === 0 ? "even" : "odd");
  return mapArray;
};
